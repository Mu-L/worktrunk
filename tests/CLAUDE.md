...
`task profile-tests` points `TMPDIR` at a fresh directory and lists what survives the run. That directory is rooted at `/tmp` rather than at a bare `mktemp -d`, which honors `$TMPDIR` and so lands one root below another — 15 bytes more than the 14 the socket budget above has spare. Only `wt/isolated-cwd` and `wt-test-profraw` may survive; anything else is a leak. Process-scoped scratch space belongs in a fixed directory, not a `TempDir` in a `static`: statics don't run destructors at process exit, so under nextest's process-per-test model that leaks one directory per test into a temp root nothing reliably sweeps (macOS clears it only at boot).
